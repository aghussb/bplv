        ...
        <div class="w-full surface-card py-8 px-5 sm:px-8" style="border-radius: 53px">
          ...
          <form @submit.prevent="submit">
            <div>
              ...
              <InputText id="email1" type="text" placeholder="Email address" class="w-full md:w-30rem mb-5" v-model="form.email" />

              <label for="password1" class="block text-900 font-medium text-xl mb-2">Password</label>
              <Password id="password1" v-model="form.password" placeholder="Password" :feedback="false" :toggleMask="true" class="w-full mb-3" inputClass="w-full"></Password>
              ...
              <Link href="./daftar" class="font-medium no-underline ml-2 cursor-pointer" style="color: var(--primary-color)">Sign Up</Link>.
            </div>
          </form>
        </div>
      </div>
  ...
  setup() {
    ...
    const submit = () => {
      ...
      router.post('/login', {
        ...
        onError: (error) => {
          toast.add({ severity: 'error', summary: 'Validation', detail: error[Object.keys(error)[0]], life: 3000 });
          return;
        }
      });
    };
